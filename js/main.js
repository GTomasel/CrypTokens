...
startButton.onclick = () => { getCoins(), navLinkColorChange('startButton') };
...
favoritesBtn.onclick = () => { favoritesButton(), navLinkColorChange('favoritesBtn') };

const searchBtn = document.getElementById('searchBtn')
searchBtn.onclick = () => searchCoin()

//Add Enter key event 
function enterKeyEvent(fieldID, functionExecute) {
    const enterKeyEvent = document.getElementById(fieldID);
    enterKeyEvent.addEventListener("keyup", function (event) {
        if (event.keyCode === 13) {
...
//Add or remove fav icon
function addFavoriteIcon(state, favStar) {
    if (state == 'add') {
...
function navLinkColorChange(id) {
    if (id == 'startButton') {
        document.getElementById('startButton').style.color = "#ffc107"
        document.getElementById('favoritesBtn').style.color = "white"
    } else if (id == 'favoritesBtn') {
        document.getElementById('startButton').style.color = "white"
        document.getElementById('favoritesBtn').style.color = "#ffc107"
    } else if (id == 'search') {
        document.getElementById('startButton').style.color = "white"
        document.getElementById('favoritesBtn').style.color = "white"
    }
...
async function getCoins(qty = 20, currency = 'usd') {
    ...
    for (let i = 0; i < qty; i++) {
        ...
        addCheckboxEventListener(checkbox, favStar);
...
async function searchCoin() {
    ...
    if (data[0] == null || id.length == 0) {
        ...
        navLinkColorChange('search')

        document.getElementById('searchField').value = '';
    }
}

enterKeyEvent("searchField", searchCoin);
...
async function drawFavorites(idCoin, id, currency = 'usd') {
    ...
    if (loggedUserCredentials !== undefined && loggedUserCredentials.logged == 1) {
        ...
        for (let i = 0; i < loggedUserCredentials.favorites.length; i++) {
            ...
            if (loggedUserCredentials.favorites[i] == coinID) {
                ...
                document.getElementById('favCheckbox' + i).checked = true;
                addFavoriteIcon('add', 'favStar' + id);
    ...
    const checkbox = document.getElementById("favCheckbox" + id);

    addCheckboxEventListener(checkbox, 'favStar' + id, 'remove', id);

}


//Add event listener to favorites checkboxes
function addCheckboxEventListener(checkbox, favStar, removeCardOnUncheck, cardRemoveID) {
    ...
    checkbox.addEventListener("click", e => {
        if (e.target.checked == true) {
            ...
            addFavoriteIcon('remove', favStar);

            if (removeCardOnUncheck == 'remove') {
                document.getElementById('card' + cardRemoveID).innerHTML = "";
            }
